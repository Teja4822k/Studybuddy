            ...
            try:
                if language == 'python':
                    ...
                    with open(file_path, 'w') as f:
                        f.write(code)
                    result['output'] = subprocess.check_output(['python3', file_path], stderr=subprocess.STDOUT, timeout=5).decode()
                    ...
                    with open(cpp_file, 'w') as f:
                        f.write(code)
                    subprocess.check_output(['g++', cpp_file, '-o', exe_file], stderr=subprocess.STDOUT, timeout=5)
                    result['output'] = subprocess.check_output([exe_file], stderr=subprocess.STDOUT, timeout=5).decode()

                elif language == 'java':
                    java_file = os.path.join(tmpdir, 'Main.java')
                    with open(java_file, 'w') as f:
                        f.write(code)
                    subprocess.check_output(['javac', java_file], stderr=subprocess.STDOUT, timeout=5)
                    result['output'] = subprocess.check_output(['java', '-cp', tmpdir, 'Main'], stderr=subprocess.STDOUT, timeout=5).decode()

                elif language == 'javascript':
                    js_file = os.path.join(tmpdir, 'script.js')
                    with open(js_file, 'w') as f:
                        f.write(code)
                    result['output'] = subprocess.check_output(['node', js_file], stderr=subprocess.STDOUT, timeout=5).decode()
